...
onlyEvenValues(nums1);
onlyEvenValues(nums2);
...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(val){
        const letters=[];
        let word='';
        letters.push(val[0], val[val.length-1]);
        word=letters.join('');
        newArr.push(word);
    });
    return newArr;
...
function find(arr, searchValue) {
    ...
    return parseInt(filteredArray.join('')) ? searchValue : undefined;
};
...
function doubleOddNumbers(arr) {
    const filteredArray = arr.filter(function(num){
        return num%2!==0;
    });
    return filteredArray.map(function(num){
        return num*2;
    });
};
